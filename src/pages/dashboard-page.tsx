...
export function DashboardPage() {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Willkommen zurück, {user?.firstname}. Hier sind Ihre Vereine.
                    </p>
                </div>
                <Link to="/clubs/new">
                    <Button>
                ...
                <Card className="bg-muted/50">
                    <CardContent className="p-4 flex items-center gap-4">
                        ...
                        >
                            ...
                            {Object.entries(CLUB_STATUS_LABELS).map(([key, label]) => (
            ...
            {loading ? (
                <div className="text-center py-12 text-muted-foreground">Lade Vereine...</div>
            ) : clubs.length === 0 ? (
                <div className="text-center py-12 border border-dashed rounded-lg">
                    <h3 className="text-lg font-medium">Keine Vereine gefunden</h3>
                    <p className="text-muted-foreground mb-4">Starten Sie, indem Sie einen neuen Verein anlegen.</p>
                    <Link to="/clubs/new">
                        <Button variant="outline">Einen Verein gründen</Button>
                    </Link>
                </div>
            ) : (
                <div className="space-y-8">
                                    ...
                                    <Card className="h-full hover:bg-muted/50 transition-colors cursor-pointer">
                                        <CardHeader className="pb-2">
                                            <div className="flex items-center justify-between space-x-4">
                                                ...
                                                <span className={cn("px-2 py-1 rounded-full text-xs font-medium", CLUB_STATUS_COLORS[club.membership.status])}>
                                        ...
                                        <CardContent>
                                            <div className="text-sm text-muted-foreground">
                                                {club.membership.address || "Keine Adresse"}
                                            </div>
                                        </CardContent>
                                    </Card>
                                </Link>
                    ...
                    {isAdmin && clubs.length >= LIMIT && (
                        <div className="flex justify-center pt-4">
                            <Button variant="outline" onClick={() => setPage(p => p + 1)}>Mehr laden</Button>
                        </div>
                    )}
                </div>
            )}
